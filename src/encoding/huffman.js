/**
 * HPACK/QPACK Huffman encoding (RFC 7541 Appendix B)
 * Shared by HTTP/2 HPACK and HTTP/3 QPACK header compression.
 *
 * This implements the static Huffman table defined in RFC 7541 for
 * compressing HTTP header field names and values.
 */

/**
 * Build Huffman decoding table from RFC 7541 Appendix B.
 * Maps bit patterns to their decoded symbol values.
 *
 * @returns {Map<string, number>} Map from bit string to symbol
 */
function buildHuffmanTable() {
    ...
    const codes = [
        ...
        [256, 0x3fffffff, 30] // EOS (End of String)
    ];

    // Build a lookup table organized by bit pattern
    const table = new Map();
    for (const [symbol, code, len] of codes) {
        const bitString = code.toString(2).padStart(len, '0');
        table.set(bitString, symbol);
    }
    return table;
}
 ...
 */
export const HUFFMAN_DECODE_TABLE = buildHuffmanTable();
 ...
 * @throws {Error} If invalid Huffman code encountered
 */
export function decodeHuffman(data) {
    ...
    for (const bit of bits) {
        current += bit;
        const symbol = HUFFMAN_DECODE_TABLE.get(current);

        if (symbol !== undefined) {
            if (symbol === 256) {
                // EOS (End of String) - stop decoding
                break;
            }
        ...
        // Max code length is 30 bits - if we exceed this, the data is invalid
