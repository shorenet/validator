...
/**
 * Extract TCP segment info from raw packet (handles Ethernet/IP headers)
 * @param {Uint8Array} data - Raw packet bytes
 * @returns {Object|null} TCP segment info or null
 */
export function extractTcpSegment(data) {
    ...
    if ((data[0] >> 4) === 4) {
        ...
        if (protocol === 6 && data.length >= ihl + 20) {
            tcpData = data.slice(ihl);
        }
    }
    // Check for IPv6 header (version 6 in high nibble)
    else if ((data[0] >> 4) === 6) {
        const nextHeader = data[6];
        if (nextHeader === 6 && data.length >= 40 + 20) {
            tcpData = data.slice(40);
        }
    }
    // Check for Ethernet frame
        ...
        if (etherType === 0x0800) {
            // IPv4
            const ipData = data.slice(14);
            if (ipData.length >= 20 && (ipData[0] >> 4) === 4) {
                const ihl = (ipData[0] & 0x0f) * 4;
                const protocol = ipData[9];
                if (protocol === 6 && ipData.length >= ihl + 20) {
                    tcpData = ipData.slice(ihl);
                }
            }
            ...
            if (ipData.length >= 40) {
                ...
                if (nextHeader === 6 && ipData.length >= 40 + 20) {
                    tcpData = ipData.slice(40);
                }
            }
        }
    }
