 ...
 * Maps bit patterns to decoded symbols.
 */
function buildHuffmanDecodeTable() {
    const table = new Map();
    for (let sym = 0; sym < HUFFMAN_ENCODE_TABLE.length; sym++) {
        const [code, len] = HUFFMAN_ENCODE_TABLE[sym];
        const key = code.toString(2).padStart(len, '0');
        table.set(key, sym);
    }
    return table;
}

const HUFFMAN_DECODE_TABLE = buildHuffmanDecodeTable();
...
/**
 * Decode a QPACK integer with the given prefix bits.
        ...
        while (offset < data.length) {
            ...
            try {
                if ((firstByte & 0x80) !== 0) {
                    ...
                    const { value: name, bytesRead: nameBytes } = decodeString(data, offset);
                    offset += nameBytes;
