...
export function parseFrame(data) {
    ...
    if (opcode >= 0x8) {
        ...
        if (!fin) {
            throw new Error('WebSocket control frames cannot be fragmented');
        }
    }
...
/**
 * Decompress permessage-deflate data.
 * Note: This requires the pako library for DEFLATE decompression.
 * If pako is not available, returns the data as-is.
...
export function decompressDeflate(data) {
    ...
    if (typeof window !== 'undefined' && window.pako) {
        ...
        try {
            return window.pako.inflateRaw(withTrailer);
        } catch (e) {
    ...
    // No pako available, return as-is
    return data;
}
